...
class AutopushSettings(object):
    ...
    """
    def add_next_month(self, today):
        ""check to see if tomorrow is a new month, in case we're a node that
        has a slow clock.""
        tomorrow = (today + datetime.timedelta(days=1))
        if tomorrow.month == today.month:
            return
        if sorted(self.message_tables.keys())[-1] != self.current_msg_month:
            return
        next_month = get_rotating_message_table(
            self._message_prefix, 0, tomorrow)
        self.message_tables[next_month.table_name] = Message(
            tomorrow, self.metrics)
    """
